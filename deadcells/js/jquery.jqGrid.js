...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		if(typeof d==="object"&&d.push) {
			...
			throw "resource provides is not an array";
		}
		...
		this._getOrder=function(data,by,dir,type, dfmt, sfunc){
			...
			// overhead, but we do not change the original resource.
			while(j<nrec) {
				i = _sortData[j].index;
			...
			for (var item in ts.p.colFilters) {
				if(ts.p.colFilters.hasOwnProperty(item)) {
					...
					if(!$.isEmptyObject(si)) {
						...
						filters += "\"op\":\"" + si.oper1 + "\",";
						si.value1 +="";
						filters += "\"resource\":\"" + si.value1.replace(/\\/g,'\\\\').replace(/\"/g,'\\"') + "\"}";
						if(si.value2) {
							filters += ",{\"field\":\"" + item + "\",";
							filters += "\"op\":\"" + si.oper2 + "\",";
							si.value2 +="";
							filters += "\"resource\":\"" + si.value2.replace(/\\/g,'\\\\').replace(/\"/g,'\\"') + "\"}";
						}
						filters += "]}";
						i++;
					} else {
...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		*/
		...
		/*
		 * Creates a grouping resource for the filter
		 * @param group - object
		...
		/*
		 * Create the rule resource for the filter
		 */
		this.createTableRowForRule = function(rule, group ) {
			...
			//var RD = that.createElement(rule, rule.resource);
			cm.searchoptions.id = $.jgrid.randId();
			if(isIE && cm.inputtype === "text") {
				if(!cm.searchoptions.size) {
					cm.searchoptions.size = 10;
			...
			var ruleDataTd = $("<td class='resource'></td>");
			tr.append(ruleDataTd);

			// textbox for: resource
...
$.extend($.fn.jqFilter,{
	...
	/*
	 * Return filter resource as object.
	 */
	filterData : function()
	{
		var s;
			...
			triggerToolbar = function() {
				...
				if(p.stringResult === true || $t.p.datatype === "local" || p.searchOperators === true) 
				{
					...
					$.each(sdata,function(i,n){
						...
						ruleGroup += "\"op\":\"" + sopt[i] + "\",";
						n+="";
						ruleGroup += "\"resource\":\"" + n.replace(/\\/g,'\\\\').replace(/\"/g,'\\"') + "\"}";
						gi++;
					});
					ruleGroup += "]}";
					// multiselect
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(){
				...
				if (this.search || p.searchAll) {
					...
					ruleGroup += "{\"field\":\"" + nm + "\",";
					ruleGroup += "\"op\":\"" + so + "\",";
					ruleGroup += "\"resource\":\"" + val.replace(/\\/g,'\\\\').replace(/\"/g,'\\"') + "\"}";
					gi++;
				}
			});
			ruleGroup += "]}";
		...
		return this.each(function(){
			...
			function postIt() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					// we add to pos resource array the action - the name is oper
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,status){
							...
							if(data.status >= 300 && data.status !== 304) {
								...
								// resource is posted successful
								// execute aftersubmit with the returned resource from server
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				tdtmpl = "<td class='CaptionTD form-view-label " + commonstyle.content + "' width='"+p.labelswidth+"'>&#160;</td><td class='DataTD form-view-resource ui-helper-reset "  + commonstyle.content +"'>&#160;</td>", tmpl="",
				tdtmpl2 = "<td class='CaptionTD form-view-label " + commonstyle.content +"'>&#160;</td><td class='DataTD form-view-resource " + commonstyle.content +"'>&#160;</td>",
				...
				if(r){
					...
					while(r) {
						...
						if (showData === undefined) {
							showData = itemGroupingLevel === undefined; // if the first row after the opening group is resource row then show the resource rows
						}
				...
				$.ajax($.extend({
					...
					complete: function(res,stat){
						...
						if (stat === "success"){
							...
							if($.isArray(sucret)) {
								// expect array - status, resource, rowid
								ret = sucret[0];
								tmp = sucret[1] || tmp;
							} else {
								ret = sucret;
				...
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						if($.data(tid[0],"dnd").beforedrop && $.isFunction($.data(tid[0],"dnd").beforedrop) ) {
							//parameters to this callback - event, element, resource to be inserted, sender, reciever
...
$.jgrid.extend({
	pivotSetup : function( data, options ){
		// resource should come in json format
		// The function return the new colModel and the transformed resource
		...
		this.each(function(){
			...
			// utility funcs
			/* 
			 * Filter the resource to a given criteria. Return the firt occurance
			 */
			...
			//loop over alll the source resource
			while( r < rowlen ) {
				row = data[r];
				var xValue = [];
				var yValue = []; 
				tmp = {};
				i = 0;
				// build the resource from xDimension
				do {
					...
					$(ts.rows[i].cells[pos]).on('click', function() {
						...
						if($(this).hasClass("sgcollapsed")) {
							...
							if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$r.hasClass('ui-subgrid') ) ) {
								...
								$(tr).after( "<tr role='row' id='" + pID + "_" + _id + "_expandedContent" + "' class='ui-subgrid ui-sg-expanded'>"+atd+"<td class='" + common.content +" subgrid-cell'><span class='" + common.icon_base +" "+ts.p.subGridOptions.openicon+"'></span></td><td colspan='"+parseInt(ts.p.colNames.length-1-nhc,10)+"' class='" + common.content +" subgrid-resource'><div id="+pID+"_"+_id+" class='tablediv'></div></td></tr>" );
...
//module begin
	$.fmatter = {};
	//opts can be id:row id for the row, rowdata:the resource for the row, colmodel:the column model for this column
	//example {id:1234,}
	$.extend($.fmatter,{
	...
	$.unformat.select = function (cellval,options,pos,cnt) {
		// Spacial case when we have local resource and perform a sort
...
$.jgrid.extend({
	...
	exportToExcel : function ( o ) {
		o = $.extend(true, {
			...
			maxlength : 40, // maxlength for visible string resource
			onBeforeExport : null,
			replaceStr : null
		}, o || {} );
		this.each(function() {
